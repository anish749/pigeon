...
it to the clipboard. Use this before creating or updating a Slack app.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return commands.RunGenerateManifest(flagsToArgs(cmd, "username", "workspace"))
	},
}

func init() {
	generateManifestCmd.Flags().String("username", "", "display name for the bot owner [required]")
	generateManifestCmd.Flags().String("workspace", "", "Slack workspace name [required]")
