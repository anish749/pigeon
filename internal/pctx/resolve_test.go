...
func testConfig() *config.Config {
	return &config.Config{
		...
		Contexts: map[string]config.Context{
			"work": {
				"gws":   config.StringOrSlice{"work@company.com"},
				"slack": config.StringOrSlice{"acme-corp"},
			},
			"personal": {
				"gws":      config.StringOrSlice{"user@gmail.com"},
				"slack":    config.StringOrSlice{"side-project"},
				"whatsapp": config.StringOrSlice{"+15551234567"},
			},
			"all-slack": {
				"slack": config.StringOrSlice{"acme-corp", "side-project"},
			},
		},
...
func TestResolveNoAccountForSource(t *testing.T) {
	...
	// work context has no whatsapp account.
...
func TestResolveLinear(t *testing.T) {
	cfg := testConfig()
	cfg.Contexts["work"]["linear"] = config.StringOrSlice{"trudy"}

	res, err := Resolve(cfg, SourceLinear, ResolveOpts{Context: "work"})
	if err != nil {
		t.Fatal(err)
	}
	if len(res.Accounts) != 1 || res.Accounts[0].Name != "trudy" {
		t.Errorf("accounts = %v, want [trudy]", res.Accounts)
	}
}
