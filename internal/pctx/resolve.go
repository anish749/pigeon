...
func resolveWithContext(cfg *config.Config, src Source, platform Platform, ctxName ContextName) (*Resolved, error) {
	...
	identifiers := contextAccounts(ctx, platform)
...
// contextAccounts returns the account identifiers for a platform from a
// ContextConfig. Returns nil if the platform has no accounts in the context.
func contextAccounts(ctx config.ContextConfig, platform Platform) []string {
	switch platform {
	case PlatformGWS:
		return ctx.GWS
	case PlatformSlack:
