...
func TestTailHandler_LiveEventsFlow(t *testing.T) {
	...
	onConnect := func() {
		...
		h.RouteEvent(NewMsg(acct, "#general", modelv1.MsgLine{
			ID:     "abc.123",
			Ts:     time.Now(),
			Sender: "alice",
			Text:   "hello world",
	...
	for _, f := range frames {
		if f["kind"] == "message" && f["id"] == "abc.123" {
			gotMsg = true
			if f["platform"] != "slack" || f["name"] != "acme" {
				t.Errorf("platform/name wrong: %+v", f)
			}
			if f["conversation"] != "#general" {
				t.Errorf("conversation wrong: %+v", f)
			}
			if f["text"] != "hello world" {
				t.Errorf("text wrong: got %v", f["text"])
			}
			if f["sender"] != "alice" {
				t.Errorf("sender wrong: got %v", f["sender"])
			}
		}
	}
	if !gotMsg {
...
func TestTailHandler_FilterScopesToAccount(t *testing.T) {
	...
	onConnect := func() {
		time.Sleep(10 * time.Millisecond)
		h.RouteEvent(NewMsg(acme, "#a", modelv1.MsgLine{ID: "1", Ts: time.Now(), Text: "from acme"}))
	...
	for _, f := range frames {
		...
		if f["name"] == "other" {
			t.Errorf("filter leaked: got event for name=other: %+v", f)
		}
	}
