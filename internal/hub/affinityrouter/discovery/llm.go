...
func (d *LLMDiscovery) Discover(ctx context.Context, signals []models.Signal) ([]DiscoveredWorkstream, error) {
	...
	// Discovery prompts are large (50K+ chars) and need more time than
	// the default per-call timeout. Use a 5-minute timeout for this call.
	discoveryCtx, cancel := context.WithTimeout(ctx, 5*time.Minute)
	defer cancel()

	var result llmDiscoverJSON
	if err := d.client.JSON(discoveryCtx, discoverySystemPrompt, prompt, &result); err != nil {
  ...
  "workstreams": [
    {
      ...
      "conversations": ["@Magnus", "#delivery-team"],
