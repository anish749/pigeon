// Package detector defines the ConversationShiftDetector interface for deciding when
// a conversation's buffered signals should be sent to the LLM classifier.
//
// Each ConversationShiftDetector instance is scoped to a single conversation. The router
...
// ConversationShiftDetector observes incoming signals for a single conversation and
...
type ConversationShiftDetector interface {
...
// Factory creates a new ConversationShiftDetector for a conversation. The factory
...
type Factory func() ConversationShiftDetector
