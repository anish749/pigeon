...
func TestListJiraIssues(t *testing.T) {
	...
	// Two projects, with issues + a stray non-jsonl + a hidden file.
	for _, fixture := range []struct {
		project, issue, content string
	}{
		{"ENG", "ENG-101", `{"type":"jira-issue","key":"ENG-101"}`},
		{"ENG", "ENG-102", `{"type":"jira-issue","key":"ENG-102"}`},
		{"OPS", "OPS-7", `{"type":"jira-issue","key":"OPS-7"}`},
	} {
		f := jd.Project(fixture.project).IssueFile(fixture.issue)
		if err := os.MkdirAll(filepath.Dir(f.Path()), 0o755); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(f.Path(), []byte(fixture.content+"\n"), 0o644); err != nil {
			t.Fatal(err)
		}
	}

	// Stray files that should NOT appear.
	for _, junk := range []string{
		filepath.Join(jd.Project("ENG").IssuesDir(), "README.md"),
		filepath.Join(jd.Project("ENG").Path(), ".sync-cursors.yaml"),
	} {
	...
	wantSuffixes := []string{
		...
		"/jira-issues/tubular/ENG/issues/ENG-102.jsonl",
		"/jira-issues/tubular/OPS/issues/OPS-7.jsonl",
	}
	...
	for _, suf := range wantSuffixes {
		...
		if !found {
			t.Errorf("missing expected issue with suffix %q in %v", suf, got)
		}
	}
}
...
func TestListJiraIssues_EmptyAccount(t *testing.T) {
	root := paths.NewDataRoot(t.TempDir())
	s := NewFSStore(root)
	acct := account.New(paths.JiraPlatform, "fresh")
	...
	if len(got) != 0 {
		t.Errorf("got %d issues from empty account, want 0", len(got))
	}
}

func TestListJiraIssues_WrongPlatform(t *testing.T) {
	root := paths.NewDataRoot(t.TempDir())
	s := NewFSStore(root)
	acct := account.New("slack", "acme")

	if _, err := s.ListJiraIssues(acct); err == nil {
		t.Error("ListJiraIssues on non-jira account should error")
	}
}
...
func TestFindJiraIssue(t *testing.T) {
	...
	// Seed two projects with one issue each.
	for _, fixture := range [...]struct{ project, issue string }{
		{"ENG", "ENG-101"},
		{"OPS", "OPS-7"},
	} {
		f := jd.Project(fixture.project).IssueFile(fixture.issue)
	...
	got, err := s.FindJiraIssue(acct, "OPS-7")
	if err != nil {
		t.Fatalf("FindJiraIssue OPS-7: %v", err)
	}
	if !strings.HasSuffix(got.Path(), "/jira-issues/tubular/OPS/issues/OPS-7.jsonl") {
		t.Errorf("FindJiraIssue OPS-7: got %q, want suffix /jira-issues/tubular/OPS/issues/OPS-7.jsonl", got.Path())
	}

	if _, err := s.FindJiraIssue(acct, "MISSING-1"); err == nil {
		t.Error("FindJiraIssue MISSING-1 should error")
	}
}

func TestFindJiraIssue_NoSyncedData(t *testing.T) {
	root := paths.NewDataRoot(t.TempDir())
	s := NewFSStore(root)
	acct := account.New(paths.JiraPlatform, "fresh")

	if _, err := s.FindJiraIssue(acct, "ENG-1"); err == nil {
		t.Error("FindJiraIssue with no synced data should error")
	}
}
