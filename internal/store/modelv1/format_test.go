...
func TestFormatReactionNotification_ConvMeta(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "parent-found, react on thread reply with channel meta (full)",
			render: func() []string {
				r := baseReact
				r.ThreadTS = "P1"
				r.ThreadID = "P1"
				return FormatReactionNotification(msg, r, time.UTC, channelMeta)
			},
			wantMeta: "  [reaction] [10:15:02] [message_id:M1] [sender_id:U001] [emoji:thumbsup] [thread_ts:P1] [thread_id:P1] [type:channel] [channel_id:C0AS]",
		},
		{
