...
// ListJiraIssues returns the issue file path for every issue under acct,
// flattened across projects. Walk: <acct>/<project>/issues/*.jsonl. Project
// directories are deliberately not exposed in the result — callers that need
// the project key can recover it from the file path or the issue key prefix.
//
// Returns nil (no error) when the account has not been synced yet — the
// account directory may not exist on first run.
func (s *FSStore) ListJiraIssues(acct account.Account) ([]paths.JiraIssueFile, error) {
	if acct.Platform != paths.JiraPlatform {
		return nil, fmt.Errorf("ListJiraIssues: account %s is not on the %s platform", acct.Display(), paths.JiraPlatform)
	}
	jd := s.root.AccountFor(acct).Jira()
	projects, err := os.ReadDir(jd.Path())
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("list jira projects in %s: %w", jd.Path(), err)
	}

	var files []paths.JiraIssueFile
	var errs []error
	for _, p := range projects {
		if !p.IsDir() || strings.HasPrefix(p.Name(), ".") {
			continue
		}
		proj := jd.Project(p.Name())
		entries, err := os.ReadDir(proj.IssuesDir())
		if err != nil {
			if os.IsNotExist(err) {
				continue
			}
			errs = append(errs, fmt.Errorf("list issues for project %s: %w", p.Name(), err))
			continue
		}
		for _, e := range entries {
			if e.IsDir() || !strings.HasSuffix(e.Name(), paths.FileExt) {
			...
			key := strings.TrimSuffix(e.Name(), paths.FileExt)
			files = append(files, proj.IssueFile(key))
		}
	}
	return files, errors.Join(errs...)
}

// FindJiraIssue locates the issue file for key under acct, scanning project
// subdirectories until it finds a match. Returns a loud not-found error when
// no project contains the issue — callers (e.g. `pigeon read`) propagate
// this directly to the user.
//
// Why scan rather than parse the key: project keys are opaque strings (Jira
// permits multi-segment keys with dashes on some installations), so we
// cannot reliably derive the project from the issue key by splitting. The
// directory walk is the source of truth for which project owns which key.
func (s *FSStore) FindJiraIssue(acct account.Account, key string) (paths.JiraIssueFile, error) {
	if acct.Platform != paths.JiraPlatform {
		return "", fmt.Errorf("FindJiraIssue: account %s is not on the %s platform", acct.Display(), paths.JiraPlatform)
	}
	jd := s.root.AccountFor(acct).Jira()
	projects, err := os.ReadDir(jd.Path())
	if err != nil {
		if os.IsNotExist(err) {
			return "", fmt.Errorf("no synced jira data for %s", acct.Display())
		}
		return "", fmt.Errorf("list jira projects in %s: %w", jd.Path(), err)
	}
	for _, p := range projects {
		if !p.IsDir() || strings.HasPrefix(p.Name(), ".") {
			continue
		}
		candidate := jd.Project(p.Name()).IssueFile(key)
		if _, err := os.Stat(candidate.Path()); err == nil {
			return candidate, nil
		} else if !os.IsNotExist(err) {
			return "", fmt.Errorf("stat %s: %w", candidate.Path(), err)
		}
	}
	return "", fmt.Errorf("jira issue %s not found in %s", key, acct.Display())
}

// ReadRaw returns the raw bytes of a log file. Intended for callers that
// need to pass-through on-disk JSONL unchanged — `pigeon read` for Jira
// uses this so the agent consumes Jira's native JSON without pigeon's
// re-rendering. Returns nil (no error) when the file does not exist; that
// path is unreachable for callers that locate the file via Find* first.
func (s *FSStore) ReadRaw(df paths.LogFile) ([]byte, error) {
	data, err := os.ReadFile(df.Path())
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil, nil
		}
		return nil, fmt.Errorf("read %s: %w", df.Path(), err)
	}
	return data, nil
}
