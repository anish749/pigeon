...
// ParsedMessage is a message parsed from a text file line.
...
// ParseMessageLine parses a formatted message line into its components.
// Format: [2026-03-16 09:15:02 -04:00] Alice: hello world
// Returns nil if the line cannot be parsed.
func ParseMessageLine(line string) *ParsedMessage {
	ts := parseLineTime(line)
	if ts.IsZero() {
		return nil
	}

	// Find the "] " after the timestamp, then parse "Sender: Text"
	trimmed := strings.TrimLeft(line, " ")
	...
	// Split on first ": " to get sender and text.
...
// ReadMessagesSince reads all messages in a conversation after the given timestamp.
// Returns message lines in chronological order. Returns nil if no messages found.
func ReadMessagesSince(platform, account, conversation string, since time.Time) ([]string, error) {
	dir := filepath.Join(DataDir(), platform, account, conversation)
	files, err := listTxtFiles(dir)
	if err != nil {
